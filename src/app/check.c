...
void doc_color_code_color_codes(char* sec, int sec_len, char* buf, int* buf_pos_ptr, int* i_ptr, bool* skip_char_ptr, bool* syntax)
{
  ...
  { 
    ...
    // P_STR(keywords[0]);
    //
    bool style_act = core_data->style_act;
    bool use_icons = core_data->use_icons;
    ...
    core_data->style_act = style_act;
    core_data->use_icons = use_icons;
    ...
    STRCPY(box_indent, core_data->text_box_indent);
    P_STR( box_indent );
    P_STR( core_data->text_box_indent );
